...
void RangeSensorLayer::onInitialize() {
    ...
    if (topic_names.size() < 1) {
        ...
        ROS_WARN(
                "Empty topic names list: range sensor layer will have no effect "
                "on costmap");
    }
    ...
    for (unsigned int i = 0; i < topic_names.size(); i++) {
        if (topic_names[i].getType() != XmlRpc::XmlRpcValue::TypeString) {
            ROS_WARN(
                    "Invalid topic names list: element %d is not a string, so it "
                    "will be ignored",
                    i);
        } else {
...
void RangeSensorLayer::updateCostmap(sensor_msgs::Range& range_message, bool clear_sensor_cone) {
    ...
    if (!tf_->waitForTransform(global_frame_, in.header.frame_id, in.header.stamp, ros::Duration(0.1))) {
        ROS_ERROR_THROTTLE(1.0, "Range sensor layer can't transform from %s to %s at %f", global_frame_.c_str(),
                in.header.frame_id.c_str(), in.header.stamp.toSec());
        return;
    }
...
void RangeSensorLayer::updateBounds(
    ...
    *max_y = std::max(*max_y, max_y_);
    ...
    if (buffered_readings_ == 0) {
        if (no_readings_timeout_ > 0.0 && (ros::Time::now() - last_reading_time_).toSec() > no_readings_timeout_) {
