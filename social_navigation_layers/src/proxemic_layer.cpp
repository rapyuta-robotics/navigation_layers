...
PLUGINLIB_EXPORT_CLASS(social_navigation_layers::ProxemicLayer,
                       costmap_2d::Layer)
...
double gaussian(double x, double y, double x0, double y0, double A, double varx,
                double vary, double skew) {
...
void ProxemicLayer::updateBoundsFromPeople(double *min_x, double *min_y,
...
void ProxemicLayer::updateCosts(costmap_2d::Costmap2D &master_grid, int min_i,
  ...
  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end();
    ...
    for (int i = start_x; i < end_x; i++) {
      for (int j = start_y; j < end_y; j++) {
        ...
        double a;
        if (fabs(diff) < M_PI / 2)
          a = gaussian(x, y, cx, cy, amplitude_, covar_ * factor, covar_,
                       angle);
        else
